...
/**
 * @description 递归处理成此后台需要的数据
 * */
export function handleInitRoutes(menuList: Menu.MenuOptionsOld[]): Menu.MenuOptionsOld[] {
  menuList.forEach(opt => {
    defaultInitFun(opt);
  });
  if(menuList.every(item => item.name !=="home")){
    // 手动添加home数据
    menuList.unshift({
...
function defaultInitFun(opt: Menu.MenuOptionsOld) {
  ...
  opt.path=opt.path.indexOf("/")===0 ? opt.path: "/"+opt.path
  if(opt.parentPath){
    opt.path=opt.parentPath+opt.path
  }
  opt.meta = {
    icon: opt.meta.icon || "",
    isAffix: false,
  ...
  delete opt.hidden;
  delete opt.alwaysShow;
  if (opt.children?.length) {
    opt.children.forEach(innerOpt => {
      innerOpt.parentPath = opt.path;
      defaultInitFun(innerOpt);
    });
  } else {
    delete opt.children;
  }
